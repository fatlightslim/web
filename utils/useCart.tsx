...
interface CartProviderState extends InitialState {
  ...
  updateItemQty: (id: Item["id"], qty: number) => void
  emptyCart: () => void
...
export const CartProvider: React.FC<{
  ...
  const addItem = (item: Item, qty = 1) => {
    ...
    const payload = { ...item, qty: currentItem.qty + qty }
  ...
  const updateItemQty = (id: Item["id"], qty: number) => {
    if (qty <= 0) {
