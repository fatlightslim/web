...
export default function Cart({ cartOpen, setCartOpen }) {
  ...
  const Actions = () => (
    <div className="flex-shrink-0 px-4 py-4 flex justify-center">
      ...
      <Link href="/order">
        <a className="ml-4 inline-flex justify-center py-2 px-4 border border-transparent shadow-sm text-sm font-medium rounded-md text-white bg-indigo-600 hover:bg-indigo-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500">
          ご購入手続き
        </a>
      </Link>
    </div>
...
const List = ({ title, image, price, id, quantity }) => {
  return (
